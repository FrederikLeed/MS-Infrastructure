...
# ------------------------------------------------------------
$TargetPath = Get-ADOrganizationalUnit -Filter "name -like '*jump*'" | Where { $_.DistinguishedName -like '*tier1*' -and $_.DistinguishedName -notlike '*limited*' }
if ($TargetPath.count -ge 1) {
    $TargetSelection = $TargetPath | Select-Object DistinguishedName | Out-GridView -Title "Select the Tier 1 JumpStations OU" -OutputMode Single
...
if ($ServersQuery.count -ge 1) {
    $SelectedServers = $ServersQuery | Select-Object -Property Name,DNSHostName,DistinguishedName | `
        Out-GridView -Title "Select the Tier 1 Jump / Management Servers" -OutputMode Multiple
} else {
    $SelectedServers = $ServersQuery
}


# Install & Configure Tier 1 management server(s)
# ------------------------------------------------------------
