...
# ------------------------------------------------------------
$TargetPath = Get-ADOrganizationalUnit -Filter "name -like '*jump*'" | Where { $_.DistinguishedName -like '*tier2*' -and $_.DistinguishedName -notlike '*limited*' }
if ($TargetPath.count -ge 1) {
    $TargetSelection = $TargetPath | Select-Object DistinguishedName | Out-GridView -Title "Select the Tier 2 JumpStations OU" -OutputMode Single
...
if ($ServersQuery.count -ge 1) {
    $SelectedServers = $ServersQuery | Select-Object -Property Name,DNSHostName,DistinguishedName | `
        Out-GridView -Title "Select the Tier 2 Jump / Management Servers" -OutputMode Multiple
} else {
    $SelectedServers = $ServersQuery
}


# Install & Configure Tier 2 management server(s)
# ------------------------------------------------------------
